...
FMT* _fmt_new_rawdata(void* data,unsigned int length)
{
	...
	fmt->_ref = 0;
	fmt->t = _PDT_RAW;
	fmt->m_str = (unsigned char *) data;
	fmt->m_slen = length;

	fmt->_delete = &fmt_base_delete;
	fmt->_serialize = &fmt_raw_serialize;
	return fmt;
}
...
int fmt_packet_build(automem_t * pmem, FMT * fmt, short cmd, unsigned int userId)
{
	if(NULL != fmt)
	{
		...
		fmt_packet_build_header(pmem, cmd, userId, dataSize);
		automem_append_voidp(pmem, mem.pdata, mem.size);
		automem_uninit(&mem);
	}
	else
	{
		fmt_packet_build_header(pmem, cmd, userId, 0);
	}
...
int fmt_packet_build_server(automem_t * pmem,FMT * fmt,short cmd)
{
	...
	if(NULL != fmt)
	{
		...
		automem_init(&mem,128);
		dbLen = fmt->_serialize(fmt,&mem);
		/*dl = */fmt_write_length(pmem,dbLen);//写入整数

		automem_append_voidp(pmem,mem.pdata,mem.size);
		automem_uninit(&mem);
	}else{
		/*int dl = */fmt_write_length(pmem,0);	//提交的数据包并未带有主体数据.
	}
...
int fmt_parser_push(FMTParserState* st,unsigned char * buffer,unsigned int size)
{
	...
	while(p < size)
	{
		...
		case __PST_VERSION:
			...
			if(FMT_PROTOCOL_VERSION != st->m_ver)
			{
		...
		case __PST_COMMAND:
			...
			break;
		case __PST_USERID:
			if(size >= p+sizeof(unsigned int))
			{
#ifdef  LITTLEENDIANNESS
				st->m_userId= SWAP_32(*(unsigned int *)&buffer[p]);
#else
				st->m_userId= *(unsigned int *)&buffer[p];
#endif
		...
		case __PST_DATA:
			...
			if((size - p) < st->m_dLen)  //数据不够,等待数据满.
				return p;
			if((0x8000 & st->m_cmd) && (st->lite))
			{
				st->result = fmt_object_get(_fmt_new_rawdata(&buffer[p], st->m_dLen));
				st->t = __PST_FINISHED;
				return p + st->m_dLen;
			}else
			{
